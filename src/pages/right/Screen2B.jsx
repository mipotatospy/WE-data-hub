...
const CATEGORY_COLORS = {
  red: { primary: "#8B1E3F", secondary: "#D6B85A" },
  white: { primary: "#D6B85A", secondary: "#A67C52" },
  rose: { primary: "#E89AAE", secondary: "#C76D8A" },
  sparkling: { primary: "#4DA6FF", secondary: "#7DCFB6" },
  special: { primary: "#7C5CFF", secondary: "#9C7BFF" },
};
...
export default function Screen2B({ categoryId, categoryLabel }) {
  ...
  if (loading) return <div>Loading Screen 2.B...</div>;
  if (error) return <div>{error}</div>;
  if (!screen2BData) return <div>No Screen 2.B data available.</div>;
  ...
  return (
    <div className="screen screen2b">
      <div className="screen-1b__header">
        <h1 className="screen-1b__title">
          {formatCategoryTitle(categoryId || categoryLabel)}
        </h1>
      </div>

      <div className="screen2b-layout">
        <section className="screen__section screen2b-top">
          <h3>Top 10 Liked Wines</h3>
          <HorizontalBarChart
            items={topLikedWines}
            color={categoryColors.primary}
        ...
        <section className="screen__section screen2b-balance">
          <h3>Top 3 Wines with Highest Balance</h3>
          <VerticalBarChart
            items={topBalanceWines}
            color={categoryColors.secondary}
        ...
        <section className="screen__section screen2b-cloud">
          <WordCloudChart
            words={descriptorCloud}
            title="Descriptor Word Cloud"
            maxWords={20}
