...
function Screen2A() {
  ...
  // Ensure empty arrays are passed if data is unavailable
  const stackedBars = screen2AData?.stackedBars || [];
  const grapePies = screen2AData?.grapePies || [];

  return (
    <div className="screen2a">
      <h1>Most Recognised Grapes</h1>
      ...
      <div className="screen2a-top">
        ...
        {/* Pass empty data to StackedVerticalBarChart if no data */}
        <StackedVerticalBarChart
          data={stackedBars.length ? stackedBars : [{ label: "No Data", correct: 0, wrong: 0 }]}
          height={550}
        />
      </div>
      ...
      <div className="screen2a-bottom">
        ...
        <div className="screen2a-pies">
          {/* Map over grapePies but ensure empty data is passed */}
          {grapePies.length ? (
            grapePies.map((grape) => {
              const correctPct = Math.round((grape.correctGuessRate ?? 0) * 100);
            ...
            // Fallback message when no pies are available
            <div>No grape pie data available.</div>
          )}
        </div>
      </div>
    </div>
