...
function mapLeaderboardItems(items = []) {
  return ensureArray(items).map((item, index) => {
    const guessCount = ensureNumber(item?.guessCount);
    const correctGuessCount = ensureNumber(item?.correctGuessCount);
    const wrongGuessCount = ensureNumber(item?.wrongGuessCount);

    return {
      grapeId: item?.grapeId || item?.grape || `grape_${index}`,
      label: item?.label || item?.grape || `Grape ${index + 1}`,
      guessCount,
      correctGuessCount,
      wrongGuessCount,
      correctGuessRate:
        item?.correctGuessRate ??
        item?.value ??
        (guessCount > 0 ? correctGuessCount / guessCount : 0),
    };
  });
}
...
function mapGrapeOverviewDocs(docs) {
  return docs.map((snap) => {
    ...
    const guessCount = ensureNumber(data.guessCount);
    const correctGuessCount = ensureNumber(data.correctGuessCount);
    const wrongGuessCount = ensureNumber(data.wrongGuessCount);

    return {
      grapeId: snap.id,
      label: data.label || data.grape || snap.id,
      guessCount,
      correctGuessCount,
      wrongGuessCount,
      correctGuessRate:
...
export async function getScreen2AData() {
  const leaderboardRef = doc(
    db,
    "leaderboards",
    "global_top_grapes_by_recognition"
  );
  ...
  if (leaderboardSnap.exists()) {
    ...
    grapes = sortByGuessCountDesc(
      mapLeaderboardItems(leaderboardData.items || [])
    ).slice(0, 10);
  } else {
    const grapeOverviewRef = collection(db, "grape_overview");
    const grapeOverviewSnap = await getDocs(grapeOverviewRef);
    ...
    grapes = sortByGuessCountDesc(
      mapGrapeOverviewDocs(grapeOverviewSnap.docs)
    ).slice(0, 10);
  }
...
function mapWineLeaderboardItems(items = [], limit) {
  return sortByValueDesc(
    ensureArray(items).map((item, index) => ({
      wineId: item?.wineId ?? null,
      label: item?.label || item?.name || `Wine ${index + 1}`,
      value: ensureNumber(item?.value),
      tastingsCount: ensureNumber(item?.tastingsCount),
    }))
  )
...
export async function getScreen2BData(categoryId) {
  ...
  const [topEnjoymentSnap, topBalanceSnap, descriptorsSnap] =
    await Promise.all([
      getDoc(topEnjoymentRef),
      getDoc(topBalanceRef),
      getDoc(descriptorsRef),
  ...
  const topEnjoymentData = topEnjoymentSnap.exists()
    ? topEnjoymentSnap.data()
    : {};

  const topBalanceData = topBalanceSnap.exists()
    ? topBalanceSnap.data()
    : {};

  const descriptorsData = descriptorsSnap.exists()
    ? descriptorsSnap.data()
    : {};
